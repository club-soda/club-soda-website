...
require_once(get_template_directory().'/functions/front-page.php');
...
//------------------------------------------------------------------------------------
// PRE-GET-POSTS ACTIONS

//------------------------------------------------------------------------------------
// Order the listing of events by the user specified date (not the publication date)

function clubsoda_order_by_meta_date( $query ) {

	if ( !is_admin() ) {

		if ( is_post_type_archive( 'clubsoda_events' ) && $query->is_main_query() ) {
		   $today = date( U, mktime(00,00,00) );
           $exclude_past_events_query = array(
				'key'       => '_clubsoda_event_date',
				'compare'   => '>=',
				'value'     => $today,
				);
			$query->set( 'meta_query', $exclude_past_events_query );

	        $query->set( 'orderby', 'meta_value_num' );
	        $query->set( 'meta_key', '_clubsoda_event_date' );
	        $query->set( 'order', 'ASC' );
	    }
	}
}
add_action( 'pre_get_posts', 'clubsoda_order_by_meta_date' );

function add_custom_post_types_archive( $query ) {
	if ( $query->is_main_query() && !is_admin() && ( is_category() || is_tag() ) )
		$query->set( 'post_type', array( 'post', 'clubsoda_events', 'clubsoda_offers', 'clubsoda_resources' ) );
	return $query;
}
