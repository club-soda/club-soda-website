 ...
 */
 ...
 ?>
 <div id="masthead-col-left" class="hide-for-small-only medium-3 columns">
	 <p></p>
 	</div>
 	<div id="masthead-col-mid" class="small-12 medium-6 columns">
 		<h1 class="assistive-text"><?php wp_title( '-', true, 'right' ); ?></h1>
 		<p class="site-title"><a href="<?php echo home_url( '/' ); ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" rel="home"><img src="<?php echo get_bloginfo('stylesheet_directory');?>/img/clubsodaweb.png" alt="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>"></a></p>
 	</div>
 	<div id="masthead-col-right" class="hide-for-small-only medium-3 columns">
 		<div class="clubsoda-tagline">
 ...
 */
 function oystershell_display_featured_image() {
 	...
 	if ( osc_is_custom_post_type() && is_single() ) {

 		$banner_image = wp_get_attachment_image( get_post_meta( get_the_ID(), '_clubsoda_banner_image_id', 1 ), 'clubsoda-banner' );
 		$profile_image = wp_get_attachment_image( get_post_meta( get_the_ID(), '_clubsoda_profile_image_id', 1 ), 'clubsoda-profile' );
 		...
 		if ( !empty( $banner_image ) ) { ?>
 			...
 			<?php if ( !empty( $profile_image ) ) { ?>
 				<div class="entry-profile-image entry-profile-image-overlay">
 					<?php echo $profile_image; ?>
 				</div>
 				<div class="clearfix"></div>
 		 	<?php }
 	 	} elseif ( !empty( $profile_image ) ) { ?>
 			<div class="entry-profile-image entry-profile-image-right">
 				<?php echo $profile_image; ?>
 			</div>
 		<?php }
 	...
 	if ( osc_is_custom_post_type() && is_archive() ) {
 ...
 */
 function oystershell_display_header_meta() {
 	...
 	if (is_attachment()) {
 		...
 		echo '<span class="meta-pubdate">';
 		oystershell_meta_pubdate();
 		clubsoda_display_post_icon();
 		echo '</span>';
 	} else {
 		echo '<span class="meta-pubdate">';
 		oystershell_meta_pubdate();
 ...
 */
 function oystershell_display_footer_meta() {
 	...
 	if (is_attachment()) {
 		...
 		clubsoda_single_meta();
 	} else {
 		clubsoda_archive_meta();
 	}
 }
endif; // oystershell_display_header_meta
 ...
 */
 function oystershell_display_postscript() { ?>
 	...
 	<section id="postscript-text" class="row postscript-text">
 		<h1 class="assistive-text"><?php _e( 'Postscript', 'oystershell' ); ?></h1>
 		<nav id="mobile-search-navigation" class="show-for-small-only small-6 small-centered columns site-navigation home-navigation" role="navigation">
 ...
 */
 function oystershell_credits() {
		...
		$options = get_option( '_clubsoda_options_general', false );

		if ( isset($options['_clubsoda_colophon']) ) {
			echo( $options['_clubsoda_colophon'] );
		} else {
			return;
		}
 	}
